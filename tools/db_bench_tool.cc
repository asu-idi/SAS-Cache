...
#include "cache/secondary_cache_cachelib.h"
...
DEFINE_int64(cache_size, 256 << 20,  // 256MB
...
DEFINE_bool(use_flash_cache_filter, false,
            "whether use filter in local flash cache");

DEFINE_bool(use_local_flash_cache, false,
            "Use the local flash as the secondary cache");

DEFINE_int64(flash_cache_size, 5120,
             "the size of local flash cache in MB");

DEFINE_int64(dram_cache_size, 8,
             "the size of local dram cache in MB");
            
DEFINE_bool(enable_flash_admission_list, false,
            "whether enable admission list of flash cache");

DEFINE_bool(enable_flash_evict_blocks, false,
            "whether enable evict invalid blocks of flash cache");

DEFINE_bool(enable_flash_prefetch_files, false,
            "whether enable prefetching compaction-generated files to flash cache");
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  std::shared_ptr<Cache> NewCache(int64_t capacity) {
    ...
    if (FLAGS_cache_type == "clock_cache") {
      ...
      if (FLAGS_use_local_flash_cache == true) {
        opts.secondary_cache = secondary_cache_;
      }
  ...
  static std::shared_ptr<SecondaryCache> NewFlashCache(uint64_t flash_size, bool enable_cache_filter, bool enable_admission_list) {
    if(FLAGS_use_local_flash_cache == false) {
      return nullptr;
    }
    bool enable_replacement = false;
    return facebook::rocks_secondary_cache::NewRocksCachelibWrapper(flash_size, enable_cache_filter, enable_admission_list, enable_replacement);
  }

 public:
  Benchmark() :
        secondary_cache_(NewFlashCache(FLAGS_flash_cache_size, FLAGS_use_flash_cache_filter, FLAGS_enable_flash_admission_list)),
    ...
    open_options_.block_cache = cache_;
    open_options_.secondary_cache = secondary_cache_;
  }
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      // read_options_.shared_cache_key = true;
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    options.enable_flash_admission_list = FLAGS_enable_flash_admission_list;
    options.enable_flash_evict_blocks = FLAGS_enable_flash_evict_blocks;
    options.enable_flash_prefetch_files = FLAGS_enable_flash_prefetch_files;
    options.env = FLAGS_env;
    options.wal_dir = FLAGS_wal_dir;
