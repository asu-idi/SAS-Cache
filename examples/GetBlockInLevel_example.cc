//
// Created by Tom cao on 2022/8/29.
//
...
#if defined(OS_WIN)
std::string primary_kDBPath = "C:\\Windows\\TEMP\\GetBlockInLevel_example_primary_db";
std::string secondary_kDBPath = "C:\\Windows\\TEMP\\GetBlockInLevel_example_secondary_db";
#else
std::string primary_kDBPath = "/tmp/GetBlockInLevel_example_primary_db";
std::string secondary_kDBPath = "/tmp/GetBlockInLevel_example_secondary_db";
#endif
...
int main(int argc, const char *argv[]) {
  ...
  std::cout << "Test : read from the same key " << std::endl;
  {
    ...
    uint64_t level = 0;
    s = primary_db->GetBlockInLevel(rocksdb::ReadOptions(), key1, level, block1);

    s = secondary_db->GetBlockInLevel(rocksdb::ReadOptions(), key1, level, block2);
