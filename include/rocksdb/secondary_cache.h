...
class SecondaryCache : public Customizable {
 public:
  ...
  virtual Status KeyRangePrefetch(std::string db_name, std::vector<char> smallest_data, std::vector<char> largest_data, int32_t level, std::shared_ptr<rocksdb::SecondaryCache> secondary_cache) {
    return Status::NotSupported("KeyRangePrefetch not supported");
  }
