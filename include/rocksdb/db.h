...
// <<<<<<< HEAD
  virtual Status GetBlockInLevel(const ReadOptions& ,
                          ColumnFamilyHandle* , const Slice& , uint64_t , 
                          std::vector<Slice>& ) {
    return Status::NotSupported();
  }
  Status GetBlockInLevel(const ReadOptions& options, const Slice& key, uint64_t level,
                  std::vector<Slice>& block_contents) {
    return GetBlockInLevel(options, DefaultColumnFamily(), key, level, block_contents);
  }


  virtual Status GetBlockInLevelRange(const ReadOptions& ,
                          ColumnFamilyHandle* , const Slice& , uint64_t , uint64_t,
                          std::vector<Slice>& ) {
    return Status::NotSupported();
  }
  Status GetBlockInLevelRange(const ReadOptions& options, const Slice& key, uint64_t low_level, uint64_t high_level,
                  std::vector<Slice>& block_contents) {
    return GetBlockInLevelRange(options, DefaultColumnFamily(), key, low_level,high_level , block_contents);
  }

  virtual Status GetAllBlocksInLevel(const ReadOptions& ,
                          ColumnFamilyHandle* , uint64_t , 
                          std::vector<Slice>& ) {
    return Status::NotSupported();
  }
  Status GetAllBlocksInLevel(const ReadOptions& options, uint64_t level,
                  std::vector<Slice>& block_contents) {
    return GetAllBlocksInLevel(options, DefaultColumnFamily(), level, block_contents);
  }

  virtual Status GetBlockInKeyRange(const ReadOptions& ,
                          ColumnFamilyHandle*, uint64_t, Slice, Slice,
                          std::vector<Slice>& ) {
    return Status::NotSupported();
  }
  Status GetBlockInKeyRange(const ReadOptions& options, uint64_t level, Slice min_key, Slice max_key, 
                  std::vector<Slice>& block_contents) {
    return GetBlockInKeyRange(options, DefaultColumnFamily(), level, min_key, max_key, block_contents);
  }

  // virtual Status GetIndexBlock(const ReadOptions& ,
  //                         ColumnFamilyHandle* , std::string,  
  //                         std::vector<std::string>& ) {
  //   return Status::NotSupported();
  // }
  // Status GetIndexBlock(const ReadOptions& options, std::string name,  
  //                 std::vector<std::string>& block_contents) {
  //   return GetIndexBlock(options, DefaultColumnFamily(), name ,  block_contents);
  // }
...
// =======
  ...
  // on error.
// >>>>>>> 08d87f9b2228570395e20779fbadb47b2f8972da
