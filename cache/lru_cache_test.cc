...
class TestSecondaryCache : public SecondaryCache {
 public:
  ...
  explicit TestSecondaryCache(size_t capacity, void* provided_addr)
      : num_inserts_(0), num_lookups_(0), inject_failure_(false) {
    cache_ =
        NewLRUCache(capacity, provided_addr, 0, false, 0.5 /* high_pri_pool_ratio */, nullptr,
                    kDefaultToAdaptiveMutex, kDontChargeCacheMetadata);
  }
...
TEST_F(LRUCacheSecondaryCacheTest, BasicTest) {
  ...
  // std::cout << "k1: " << k1.AsSlice().data() << std::endl;
  // std::cout << "k2: " << k2.AsSlice().data() << std::endl;
  // std::string k1 = "hello1";
  // std::string k2 = "hello2";
...
// Warning: `TestGetBlockWithLevelWithSecondaryCache`, `TestGetBlockWithKeyRangeWithSecondaryCache` and `TestGetBlockWithLevelWithDifferentProcess` can not run both, you can run them seperatly, They all can pass.The reason I still don't know, I am sure it's on gtest.

// secondary DB and primary DB share the Secondary Cache and block cache(maybe test without later).
// Use secondary DB to fetch the blocks and insert it to the secondary Cache(CacheTire == Only)
// Then we use primary DB to fetch
// In this test, the block cache capacity is enough for the two block and we test GetBlockWithKeyLevel
// TEST_F(DBSecondaryCacheTest, TestGetBlockWithLevelWithSecondaryCache) {
//   LRUCacheOptions opts(8 * 1024 /* capacity */, 0 /* num_shard_bits */,
//                        false /* strict_capacity_limit */,
//                        0.5 /* high_pri_pool_ratio */,
//                        nullptr /* memory_allocator */, kDefaultToAdaptiveMutex,
...
//   options.env = fault_env_.get();
//   // options.paranoid_file_checks = true;
//   std::string dbname1 = test::PerThreadDBPath("db_t_tc_1");
//   ASSERT_OK(DestroyDB(dbname1, options));
//   DB* db1 = nullptr;
//   ASSERT_OK(DB::Open(options, dbname1, &db1));
//   std::string dbname2 = test::PerThreadDBPath("db_t_tc_2");
//   ASSERT_OK(DestroyDB(dbname2, options));
//   DB* db2 = nullptr;
//   Options options2 = options;
//   options2.lowest_used_cache_tier = CacheTier::kOnlyNonVolatileBlockTier;
//   ASSERT_OK(DB::OpenAsSecondary(options2, db1->GetName(), dbname2, &db2));
//   fault_fs_->SetFailGetUniqueId(true);

//   // First use primary DB to write data and then flush. There will generate two blocks.
//   WriteOptions wo;
//   Random rnd(301);
//   const int N = 6;
//   for (int i = 0; i < N; i++) {
...
//   ASSERT_EQ(secondary_cache->num_lookups(), 0u);
//   ASSERT_OK(db1->Flush(FlushOptions()));
//   // At this time, in the block cache, the block is the block2 which contains the key(4) and key(5)

//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 0u);

//   // No change
//   ASSERT_OK(db2->TryCatchUpWithPrimary());
//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 0u);
...
//   // Use secondary DB to fetch blocks and it will insert to secondary cache.
//   std::vector<std::string> blocks;

//   auto key = Key(0);
//   std::thread get_block1(static_cast<Status(DB::*)(const ReadOptions&, const Slice&, uint64_t, std::vector<std::string>&)>(&DB::GetBlockInLevel), db2, std::ref(ro), key, 0, std::ref(blocks));
//   // ASSERT_OK(db2->GetBlockInLevel(ro, Key(0), 0, blocks));

//   key = Key(4);
//   std::thread get_block2(static_cast<Status(DB::*)(const ReadOptions&, const Slice&, uint64_t, std::vector<std::string>&)>(&DB::GetBlockInLevel), db2, std::ref(ro), key, 0, std::ref(blocks));

//   get_block1.join();
//   get_block2.join();

//   // insert block1 and block2
//   ASSERT_EQ(secondary_cache->num_inserts(), 2u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 0u);

//   ASSERT_OK(db1->Get(ro, Key(1), &v));
//   ASSERT_EQ(1007, v.size());
//   // This will read the block1 from secondary cache 
//   ASSERT_EQ(secondary_cache->num_inserts(), 2u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 1u);

//   // block1 is already in the block cache, these all hits, so no changes
//   ASSERT_OK(db1->Get(ro, Key(2), &v));
//   ASSERT_EQ(1007, v.size());
//   ASSERT_OK(db1->Get(ro, Key(3), &v));
//   ASSERT_EQ(1007, v.size());
//   ASSERT_EQ(secondary_cache->num_inserts(), 2u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 1u);

//   ASSERT_OK(db1->Get(ro, Key(4), &v));
//   ASSERT_EQ(1007, v.size());
//   ASSERT_OK(db1->Get(ro, Key(5), &v));
//   ASSERT_EQ(1007, v.size());

//   // It will try to read from secondary cache, the size of block cache is 
//   // enough for the two blocks.
//   ASSERT_EQ(secondary_cache->num_inserts(), 2u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 2u);
...
// Use secondary DB to fetch the blocks and insert it to the secondary Cache(CacheTire == Only)
// Then we use primary DB to fetch
// TEST_F(DBSecondaryCacheTest, TestGetBlockWithKeyRangeWithSecondaryCache) {
//   LRUCacheOptions opts(4 * 1024 /* capacity */, 0 /* num_shard_bits */,
//                        false /* strict_capacity_limit */,
//                        0.5 /* high_pri_pool_ratio */,
//                        nullptr /* memory_allocator */, kDefaultToAdaptiveMutex,
...
//   options.env = fault_env_.get();
//   options.paranoid_file_checks = true;
//   std::string dbname1 = test::PerThreadDBPath("db_t_c_1");
//   ASSERT_OK(DestroyDB(dbname1, options));
//   DB* db1 = nullptr;
//   ASSERT_OK(DB::Open(options, dbname1, &db1));
//   std::string dbname2 = test::PerThreadDBPath("db_t_c_2");
//   ASSERT_OK(DestroyDB(dbname2, options));
//   DB* db2 = nullptr;
//   Options options2 = options;
//   options2.lowest_used_cache_tier = CacheTier::kOnlyNonVolatileBlockTier;
...
//   ASSERT_OK(db1->Get(ro, Key(4), &v));
//   ASSERT_EQ(1007, v.size());
//   // This all hit in the block cache , so there is no change.
//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 2u);
...
//   ASSERT_OK(db2->GetBlockInKeyRange(rocksdb::ReadOptions(), 0, Key(0), Key(N-1), blocks));
...
// Use fork a process and in the child process, use secondary db to fetch block, because the addr space of the child and parent 
// is different, so the parent process(primary_db) can not access the prefetched block.And we use shared memory to allow child
// and parent process to communicate
// TEST_F(DBSecondaryCacheTest, TestGetBlockWithLevelWithDifferentProcess) {
//   LRUCacheOptions opts(8 * 1024 /* capacity */, 0 /* num_shard_bits */,
//                        false /* strict_capacity_limit */,
//                        0.5 /* high_pri_pool_ratio */,
//                        nullptr /* memory_allocator */, kDefaultToAdaptiveMutex,
...
//   struct shm_remove  
//   {  
//       shm_remove() { boost::interprocess::shared_memory_object::remove("secondary_cache"); }  
//       ~shm_remove(){ boost::interprocess::shared_memory_object::remove("secondary_cache"); }  
//   } remover;  

//   boost::interprocess::shared_memory_object shm_obj(
//   boost::interprocess::create_only, "secondary_cache", boost::interprocess::read_write);  

//   shm_obj.truncate(sizeof(LRUCacheShard) * (1 << 0)); // This is the default size of secondary cache
//   boost::interprocess::mapped_region region(shm_obj, boost::interprocess::read_write);

//   std::shared_ptr<TestSecondaryCache> secondary_cache(
//       new TestSecondaryCache(2048 * 1024, region.get_address()));
//   opts.secondary_cache = secondary_cache;
//   std::shared_ptr<Cache> cache = NewLRUCache(opts);
//   BlockBasedTableOptions table_options;
//   table_options.block_cache = cache;
...
//   options.env = fault_env_.get();
//   // options.paranoid_file_checks = true;
//   std::string dbname1 = test::PerThreadDBPath("db_t_th_1");
//   ASSERT_OK(DestroyDB(dbname1, options));
//   DB* db1 = nullptr;
//   ASSERT_OK(DB::Open(options, dbname1, &db1));
//   fault_fs_->SetFailGetUniqueId(true);
...
//   if(pid == 0) {
//     boost::interprocess::shared_memory_object shm(boost::interprocess::open_only, "secondary_cache", boost::interprocess::read_write);  

//     boost::interprocess::mapped_region region2(shm, boost::interprocess::read_write);

//     std::shared_ptr<TestSecondaryCache> secondary_cache2(
//         new TestSecondaryCache(2048 * 1024, region2.get_address()));

//     opts.secondary_cache = secondary_cache2;
//     std::shared_ptr<Cache> cache2 = NewLRUCache(opts);
//     BlockBasedTableOptions table_options2;
//     table_options2.block_cache = cache2;
//     table_options2.block_size = 4 * 1024;
//     table_options2.use_shared_cache_key = true;
//     Options options2 = GetDefaultOptions();
//     options2.create_if_missing = true;
//     options2.table_factory.reset(NewBlockBasedTableFactory(table_options));
//     options2.env = fault_env_.get();
//     options2.lowest_used_cache_tier = CacheTier::kOnlyNonVolatileBlockTier;

//     std::string dbname2 = test::PerThreadDBPath("db_t_th_2");
//     ASSERT_OK(DestroyDB(dbname2, options2));
//     DB* db2 = nullptr;
//     ASSERT_OK(DB::OpenAsSecondary(options2, db1->GetName(), dbname2, &db2));

//     ASSERT_OK(db2->TryCatchUpWithPrimary());
//     // child process , just used to read block
//     std::vector<std::string> blocks;
//     std::cout << "begine GetBlock" << std::endl;
//     ASSERT_OK(db2->GetBlockInLevel(ro, Key(0), 0, blocks));
//     ASSERT_OK(db2->GetBlockInLevel(ro, Key(4), 0, blocks));
...
//     // boost::interprocess::shared_memory_object::remove("secondary_cache"); // to enable we clean it
//     exit(0);
//   }
//   pid = wait(0);
...
//   // It will try to read from secondary cache, there still no and the block cache capacity is enough for two blocks
//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 2u);
...
// TEST_F(DBSecondaryCacheTest, TestGetBlockWithLevelWithIndexBlock) {
//   LRUCacheOptions opts(8 * 1024 /* capacity */, 0 /* num_shard_bits */,
//                        false /* strict_capacity_limit */,
//                        0.5 /* high_pri_pool_ratio */,
//                        nullptr /* memory_allocator */, kDefaultToAdaptiveMutex,
...
//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   // no recover ,so no read index block 
//   ASSERT_EQ(secondary_cache->num_lookups(), 0u);
...
//   ASSERT_EQ(secondary_cache->num_inserts(), 0u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 1u); // why? read index block? try to figure out why
...
//   // insert block1 and index block
//   ASSERT_EQ(secondary_cache->num_inserts(), 4u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 1u);

//   // Here there is a index block in block cache
//   // block1 and index block in secondary cache.

//   ASSERT_OK(db1->Get(ro, Key(1), &v));
//   ASSERT_EQ(1007, v.size());
//   // This will read the block1 and index block from secondary cache 
//   ASSERT_EQ(secondary_cache->num_inserts(), 4u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 2u);
...
//   // It will try to read from secondary cache, and evict block1. 
//   ASSERT_EQ(secondary_cache->num_inserts(), 4u);
//   ASSERT_EQ(secondary_cache->num_lookups(), 3u);
