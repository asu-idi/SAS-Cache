...
class CacheKey {
 public:
  ...
  // by other means.
  inline CacheKey() : file_num_etc64_(), offset_etc64_(), file_num_() {}
  ...
  uint64_t GetFileNum() const { return file_num_; }
 ...
 protected:
  ...
  CacheKey(uint64_t file_num_etc64, uint64_t offset_etc64, uint64_t file_num)
      : file_num_etc64_(file_num_etc64), offset_etc64_(offset_etc64), file_num_(file_num) {}
...
class OffsetableCacheKey : private CacheKey {
 public:
  ...
  inline CacheKey WithOffset(uint64_t offset) const {
    assert(!IsEmpty());
    return CacheKey(file_num_etc64_, offset_etc64_ ^ offset, file_num_);
  }
