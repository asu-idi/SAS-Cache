...
Status LRUCacheShard::InsertItem(LRUHandle* e, Cache::Handle** handle,
  ...
  // std::cout << "key(in InsertItem): " 
  // << e->key().data() << " size is: " << e->key().size() 
  // << std::endl;
  {
    DMutexLock l(mutex_);
    ...
    if ((usage_ + e->total_charge) > capacity_ &&
      ...
      if (handle == nullptr) {
        ...
        // std::cout << "e key is: " << e->key().data() << std::endl;
      ...
      if (old != nullptr) {
        ...
        if (!old->HasRefs()) {
          ...
          assert(usage_ >= old->total_charge);
          usage_ -= old->total_charge;
          // std::cout << "push old" << std::endl;
          // std::cout << "old key is: " << old->key().data() << std::endl;
          last_reference_list.push_back(old);
        }
      }
      if (handle == nullptr) {
  ...
  // std::cout << "InsertItem Time: " << duration << "us" << std::endl;
  return s;
}
...
Cache::Handle* LRUCacheShard::Lookup(
  ...
  auto start_time = std::chrono::high_resolution_clock::now();
  // std::cout << "key(in LRUCache): " << key.data() << std::endl;
  LRUHandle* e = nullptr;
  ...
  auto end_time = std::chrono::high_resolution_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
  // std::cout << "Lookup Time: " << duration << "us" << std::endl;
...
Status LRUCacheShard::Insert(const Slice& key, uint32_t hash, void* value,
  ...
  // std::cout << "key(in LRUCache): " << key.data() << "size is: " << key.size() << std::endl;
  ...
  // std::cout << "e->key(in LRUCache): " << e->key().data() << "size is: " << e->key().size() << std::endl;
  return InsertItem(e, handle, /* free_handle_on_fail */ true);
}

Status LRUCacheShard::InsertToSecondaryCache(const Slice& key, uint32_t hash, void* value,
                             size_t charge,
                             void (*deleter)(const Slice& key, void* value),
                             const Cache::CacheItemHelper* helper, Cache::Priority priority) {
  // Allocate the memory here outside of the mutex.
  // If the cache is full, we'll have to release it.
  // It shouldn't happen very often though.
  LRUHandle* e = reinterpret_cast<LRUHandle*>(
  ...
  memcpy(e->key_data, key.data(), key.size());
  e->CalcTotalCharge(charge, metadata_charge_policy_);
  autovector<LRUHandle*> handle_list;
  handle_list.push_back(e);

  TryInsertIntoSecondaryCache(handle_list);
  return Status::OK();
}
...
// We can place the lru cache addr as we want, this is used to test inter-process
LRUCache::LRUCache(size_t capacity, int num_shard_bits,
                   bool strict_capacity_limit, double high_pri_pool_ratio,
                   double low_pri_pool_ratio, void* provided_addr,
                   std::shared_ptr<MemoryAllocator> allocator,
                   bool use_adaptive_mutex,
                   CacheMetadataChargePolicy metadata_charge_policy,
                   const std::shared_ptr<SecondaryCache>& secondary_cache)
    : ShardedCache(capacity, num_shard_bits, strict_capacity_limit,
                   std::move(allocator)) {
  num_shards_ = 1 << num_shard_bits;
  shards_ = reinterpret_cast<LRUCacheShard*>(provided_addr);
  size_t per_shard = (capacity + (num_shards_ - 1)) / num_shards_;
  for (int i = 0; i < num_shards_; i++) {
    new (&shards_[i]) LRUCacheShard(
        per_shard, strict_capacity_limit, high_pri_pool_ratio,
...
std::shared_ptr<Cache> NewLRUCache(
    size_t capacity, void* provided_addr, int num_shard_bits, bool strict_capacity_limit,
    double high_pri_pool_ratio,
    std::shared_ptr<MemoryAllocator> memory_allocator, bool use_adaptive_mutex,
    CacheMetadataChargePolicy metadata_charge_policy,
    const std::shared_ptr<SecondaryCache>& secondary_cache,
  ...
  return std::make_shared<LRUCache>(
      capacity, num_shard_bits, strict_capacity_limit, high_pri_pool_ratio,
      low_pri_pool_ratio, provided_addr, std::move(memory_allocator), use_adaptive_mutex,
      metadata_charge_policy, secondary_cache);
}
...
std::shared_ptr<Cache> NewLRUCache(
    ...
    CacheMetadataChargePolicy metadata_charge_policy,
    double low_pri_pool_ratio) {
  return NewLRUCache(capacity, provided_addr, num_shard_bits, strict_capacity_limit,
                     high_pri_pool_ratio, memory_allocator, use_adaptive_mutex,
                     metadata_charge_policy, nullptr, low_pri_pool_ratio);
}
}  // namespace ROCKSDB_NAMESPACE
