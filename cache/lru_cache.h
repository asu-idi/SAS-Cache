...
class ALIGN_AS(CACHE_LINE_SIZE) LRUCacheShard final : public CacheShard {
 public:
  ...
  virtual Status InsertToSecondaryCache(const Slice& key, uint32_t hash, void* value,
                        const Cache::CacheItemHelper* helper, size_t charge,
                        Cache::Priority priority) override {
    assert(helper);
    return InsertToSecondaryCache(key, hash, value, charge, nullptr, helper, priority);
  }
 ...
 private:
  ...
  Status InsertToSecondaryCache(const Slice& key, uint32_t hash, void* value, size_t charge,
                DeleterFn deleter, const Cache::CacheItemHelper* helper, Cache::Priority priority);
...
class LRUCache
 ...
 public:
  ...
  LRUCache(size_t capacity, int num_shard_bits, bool strict_capacity_limit,
           double high_pri_pool_ratio, double low_pri_pool_ratio, void* provided_addr, 
           std::shared_ptr<MemoryAllocator> memory_allocator = nullptr,
           bool use_adaptive_mutex = kDefaultToAdaptiveMutex,
           CacheMetadataChargePolicy metadata_charge_policy =
           kDontChargeCacheMetadata, const std::shared_ptr<SecondaryCache>& 
