...
Status ShardedCache::InsertToSecondaryCache(const Slice& key, void* value,
                            const CacheItemHelper* helper, size_t charge, Priority priority) {
  uint32_t hash = HashSlice(key);
  if (!helper) {
    return Status::InvalidArgument();
  }
  return GetShard(Shard(hash))
      ->InsertToSecondaryCache(key, hash, value, helper, charge, priority);
}
