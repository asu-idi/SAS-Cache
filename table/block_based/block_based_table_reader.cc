...
Status BlockBasedTable::InsertEntryToCache(
  ...
  else if(cache_tier == CacheTier::kVolatileTier) {
    s = block_cache->Insert(key, block_holder.get(), charge,
                            cache_helper->del_cb, cache_handle, priority);
  }
  else if(cache_tier == CacheTier::kOnlyNonVolatileBlockTier) {
    s = block_cache->InsertToSecondaryCache(key, block_holder.get(), cache_helper, charge,
                            priority);
  }
  return s;
}
...
void BlockBasedTable::SetupBaseCacheKey(const TableProperties* properties,
  ...
  // assert(!db_id.empty());
  // std::cout << "db_id: " << db_id << " " 
  //   << "db_session_id: " << db_session_id << " " 
  //   << "file_num: " << file_num << std::endl;
...
Status BlockBasedTable::Open(
  ...
  // If set shared_cache_key, then only the file_num is used to identify a cache key
  if(table_options.use_shared_cache_key) {
    //std::cout << "set new-cache key" << std::endl;
    SetupBaseCacheKey(rep->table_properties.get(),
                      cur_file_num, &rep->base_cache_key);
  }
  else {
    //std::cout << "set defalut-cache key" << std::endl;
  ...
  // std::cout << "del range block" << std::endl;
...
Status BlockBasedTable::PrefetchIndexAndFilterBlocks(
  ...
  if (rep_->filter_policy) {
    // std::cout << "filter block" << std::endl;
  ...
  if (!rep_->compression_dict_handle.IsNull()) {
    ...
    // std::cout << "compress block" << std::endl;
...
Status BlockBasedTable::PutDataBlockToCache(
  ...
  if (block_cache != nullptr && block_holder->own_bytes()) {
    ...
    s = InsertEntryToCache(
        ...
        BlocklikeTraits<TBlocklike>::GetCacheItemHelper(block_type),
        block_holder, charge, &cache_handle, priority);
    if (s.ok() && rep_->ioptions.lowest_used_cache_tier != CacheTier::kOnlyNonVolatileBlockTier) {
      assert(cache_handle != nullptr);
...
template <typename TBlocklike>
Status BlockBasedTable::PutDataBlockToSecondaryCache(
    const Slice& cache_key, Cache* block_cache, Cache* block_cache_compressed,
    CachableEntry<TBlocklike>* cached_block, BlockContents* raw_block_contents,
    CompressionType raw_block_comp_type,
    const UncompressionDict& uncompression_dict,
  ...
  if (block_cache_compressed != nullptr &&
    ...
    std::unique_ptr<BlockContents> block_cont_for_comp_cache(
        new BlockContents(std::move(*raw_block_contents)));
    s = InsertEntryToCache(
        CacheTier::kOnlyNonVolatileBlockTier , block_cache_compressed,
        cache_key,
        BlocklikeTraits<BlockContents>::GetCacheItemHelper(block_type),
        block_cont_for_comp_cache,
        block_cont_for_comp_cache->ApproximateMemoryUsage(), nullptr,
  ...
  if (block_cache != nullptr && block_holder->own_bytes()) {
    ...
    Cache::Handle* cache_handle = nullptr;
    s = InsertEntryToCache(
        CacheTier::kOnlyNonVolatileBlockTier, block_cache, cache_key,
        BlocklikeTraits<TBlocklike>::GetCacheItemHelper(block_type),
        block_holder, charge, &cache_handle, priority);
...
Status BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  // std::cout << "block cache addr: " << block_cache << std::endl;
  // std::cout << "compressed_block cache addr: " << block_cache_compressed << std::endl;
  ...
  bool judge_read_from_file = false; // for testing
  if (block_cache != nullptr || block_cache_compressed != nullptr) {
    // create key for block cache
    key_data = GetCacheKey(rep_->base_cache_key, handle);
    key = key_data.AsSlice();
    // std::cout << "cache key: " << key.ToString() << std::endl;

    // if (!contents) {
    if (!contents && (get_context == nullptr || (get_context != nullptr && get_context->whether_get_block() == false))) {
      // std::cout << "enter get from cache" << std::endl;
      judge_read_from_file = true;
      s = GetDataBlockFromCache(key, block_cache, block_cache_compressed, ro,
                                block_entry, uncompression_dict, block_type,
                                wait, get_context);
      // std::cout << "block: " << block_entry->GetValue() << std::endl;
    ...
    if (block_entry->GetValue() == nullptr &&
      ...
      if(judge_read_from_file) {
        // std::cout << "enter get from file" << std::endl;
      }
      ...
      if (!contents) {
        ...
        if(get_context != nullptr && get_context->whether_get_block() == true) {
          // // auto block_raw_content = contents->data;
          // // std::cout << "block_based_table_reader.cc: content: " << contents->data.data() << std::endl;
          // Slice block_raw_content = Slice(contents->data.data(), contents->data.size());
          // // std::cout << "Get block : " << block_raw_content << std::endl;
          // get_context->set_str_block_content(block_raw_content);
          // get_context->set_str_block_content(key);
        }
        ...
        // std::cout << "Storage Get Time: " << duration << "us" << std::endl;
      } else {
        raw_block_comp_type = GetBlockCompressionType(*contents);
      }

      if (s.ok() && (get_context == nullptr || (get_context != nullptr && get_context->whether_get_del_range_block() == false))) {
        if(get_context != nullptr &&  get_context->whether_get_block() == true) {
          s = PutDataBlockToSecondaryCache(
          ...
          // block to the cache.
          // std::cout << "enter load to cache" << std::endl;
...
Status BlockBasedTable::GetBlocksInOneFile(GetContext* get_context) {
  ...
  for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid(); blockhandles_iter->Next()) {
    ...
    datablock_iter.reset(NewDataBlockIterator<DataBlockIter>(
        ...
        /*get_context=*/get_context, /*lookup_context=*/nullptr,
...
Status BlockBasedTable::GetBlockKeysInOneFile(std::vector<CacheKey>& keys) {
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(ReadOptions(), /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
  ...
  for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid(); blockhandles_iter->Next()) {
    ...
    auto handle = blockhandles_iter->value().handle;

    auto key = GetCacheKey(rep_->base_cache_key, handle);
...
Status BlockBasedTable::GetBlocksWithKeyRange(GetContext* get_context, const Slice& min_key, const Slice& max_key
  , bool has_from, bool has_to) {
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(ReadOptions(), /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
  ...
  for (; blockhandles_iter->Valid(); blockhandles_iter->Next()) {
    s = blockhandles_iter->status();
    if (!s.ok()) {
      break;
    }

    // already user_key, so no need to turn it into user key.
    auto key = blockhandles_iter->key();
    // auto result1 = UserComparatorWrapper(rep_->internal_comparator.user_comparator())
    //             .CompareWithoutTimestamp(key,min_tmp_key);
    if (has_from && 
            BytewiseComparator()->Compare(key, ExtractUserKey(min_key)) < 0) {
      continue;
    }
    std::unique_ptr<InternalIterator> datablock_iter;
    Status tmp_status;
    ...
    // auto result2 = UserComparatorWrapper(rep_->internal_comparator.user_comparator())
    //             .CompareWithoutTimestamp(key,max_tmp_key);
    if (has_to && 
          BytewiseComparator()->Compare(key, ExtractUserKey(max_key)) >= 0) {
      break;
    }
  }
  return s;
...
Status BlockBasedTable::GetIndexBlock(GetContext* get_context) {
  Status s;
  auto index_handle = rep_->footer.index_handle();
  // First get the index block 
  // BlockContents index_contents;
  bool use_cache = true;
  CachableEntry<Block> index_block;
  s = RetrieveBlock(
      nullptr, ReadOptions(), index_handle,
      UncompressionDict::GetEmptyDict(), &index_block, BlockType::kIndex,
      get_context, nullptr, /* for_compaction */ false, use_cache,
      /* wait_for_cache */ true, /* async_read */ false);

  // This case has the partitioned index block, we need to read it first and then get every index block
  // Option is based on primary db, so secondary db whether can get this ?
  if(rep_->index_type == BlockBasedTableOptions::IndexType::kTwoLevelIndexSearch) {
    IndexBlockIter biter;
    index_block.GetValue()->NewIndexIterator(
        rep_->internal_comparator.user_comparator(),
        rep_->get_global_seqno(BlockType::kIndex), &biter, nullptr, true,
        rep_->index_has_first_key, rep_->index_key_includes_seq, rep_->index_value_is_full);

    biter.SeekToFirst();

    CachableEntry<Block> partitioned_index_block;
    for(; biter.Valid(); biter.Next()) {
      s = RetrieveBlock(
          nullptr, ReadOptions(), biter.value().handle,
          UncompressionDict::GetEmptyDict(), &partitioned_index_block, BlockType::kIndex,
          get_context, nullptr, /* for_compaction */ false, use_cache,
          /* wait_for_cache */ true, /* async_read */ false);
    }
...
Status BlockBasedTable::GetFilterBlock(GetContext* get_context) {
  Status s;
  auto filter_handle = rep_->filter_handle;
  bool use_cache = true;
  CachableEntry<Block> filter_block;

  // This case has the partitioned filter block, we need to read it first and then get every filter block
  // Option is based on primary db, so secondary db whether can get this ? Yes
  if(rep_->filter_type == Rep::FilterType::kPartitionedFilter) {
    s = RetrieveBlock(
        nullptr, ReadOptions(), filter_handle,
        UncompressionDict::GetEmptyDict(), &filter_block, BlockType::kFilterPartitionIndex,
        get_context, nullptr, /* for_compaction */ false, use_cache,
        /* wait_for_cache */ true, /* async_read */ false);
    ...
    filter_block.GetValue()->NewIndexIterator(
        rep_->internal_comparator.user_comparator(),
        rep_->get_global_seqno(BlockType::kFilterPartitionIndex), &biter, nullptr, true,
        rep_->index_has_first_key, rep_->index_key_includes_seq, rep_->index_value_is_full);


    biter.SeekToFirst();

    CachableEntry<Block> partitioned_filter_block;
    for(; biter.Valid(); biter.Next()) {
      s = RetrieveBlock(
          nullptr, ReadOptions(), biter.value().handle,
          UncompressionDict::GetEmptyDict(), &partitioned_filter_block, BlockType::kFilter,
          get_context, nullptr, /* for_compaction */ false, use_cache,
          /* wait_for_cache */ true, /* async_read */ false);
    }
  }
  else if(rep_->filter_type == Rep::FilterType::kFullFilter) {
    // std::cout << "FullFilter" << std::endl;
    s = RetrieveBlock(
        nullptr, ReadOptions(), filter_handle,
  ...
  // NoFilter just do nothing.
  return s;
}


Status BlockBasedTable::GetDelRangeBlock(GetContext* get_context) {
  Status s;
  auto meta_handle = rep_->footer.metaindex_handle();
  // First get the meta index block 
  bool use_cache = true;
  CachableEntry<Block> meta_index_block;
  s = RetrieveBlock(
      nullptr, ReadOptions(), meta_handle,
      UncompressionDict::GetEmptyDict(), &meta_index_block, BlockType::kMetaIndex,
      get_context, nullptr, /* for_compaction */ false, use_cache,
      /* wait_for_cache */ true, /* async_read */ false);

  IndexBlockIter biter;
  meta_index_block.GetValue()->NewIndexIterator(
      rep_->internal_comparator.user_comparator(),
      rep_->get_global_seqno(BlockType::kMetaIndex), &biter, nullptr, true,
      rep_->index_has_first_key, rep_->index_key_includes_seq, rep_->index_value_is_full);

  biter.SeekToFirst();

  CachableEntry<Block> Del_range_block;
  for(; biter.Valid(); biter.Next()) {
    if(biter.key() == kRangeDelBlockName) {
      s = RetrieveBlock(
          nullptr, ReadOptions(), biter.value().handle,
          UncompressionDict::GetEmptyDict(), &Del_range_block, BlockType::kRangeDeletion,
          get_context, nullptr, /* for_compaction */ false, use_cache,
          /* wait_for_cache */ true, /* async_read */ false);
    }
  }
