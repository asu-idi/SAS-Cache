...
class BlockBasedTable : public TableReader {
 public:
  ...
  Status GetBlocksInOneFile(GetContext* get_context) override;

  Status GetBlockKeysInOneFile(std::vector<CacheKey>& keys) override;

  Status GetBlocksWithKeyRange(GetContext* get_context, const Slice& min_key, 
  const Slice& max_key, bool has_from, bool has_to) override;

  Status GetIndexBlock(GetContext* get_context) override;

  // Status GetIndexBlock(const ReadOptions& ro, GetContext* get_context) override;

  Status GetFilterBlock(GetContext* get_context) override;

  Status GetDelRangeBlock(GetContext* get_context) override;
 ...
 private:
  ...
  template <typename TBlocklike>
  Status PutDataBlockToSecondaryCache(const Slice& cache_key, Cache* block_cache,
                                      Cache* block_cache_compressed,
                                      CachableEntry<TBlocklike>* cached_block,
                                      BlockContents* raw_block_contents,
                                      CompressionType raw_block_comp_type,
  ...
  Status GetAllBlocksInLevel(std::ostream& out_stream);
