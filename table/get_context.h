...
class GetContext {
 public:
  ...
  bool whether_get_block() {
    return this->Get_Block;
  }

  bool whether_get_del_range_block() {
    return this->GetDelRange;
  }

  void set_whether_get_block(bool option) {
    this->Get_Block = option;
  }

  void set_whether_get_del_range_block(bool option) {
    this->GetDelRange = option;
  }

  void set_str_block_content(Slice& content) {
    if(!content.empty()) {
      Slice block = Slice(content.data(), content.size());
      this->str_block_contents.emplace_back(std::move(block));
      if(content != this->str_block_contents.back()) {
        std::cout << "Error: set_str_block_content failed" << std::endl;
      }
    }
  }

  size_t get_str_block_size() {
    return str_block_contents.size();
  }

  std::vector<Slice>&& get_str_block_content() {
    return std::move(this->str_block_contents);
  }
 ...
 private:
  ...
  std::vector<Slice> str_block_contents;

  bool Get_Block;
  bool GetDelRange; //To prevent load del range block to cache which will cause bug.
};
