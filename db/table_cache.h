...
class TableCache {
 public:
  ...
  Status GetAllBlocksWithKeyRange(
      const ReadOptions& options,
      const InternalKeyComparator& internal_comparator,
  ...
  // Get any entry for the specified file number
  static void GetTableByFileNum(Cache* cache, uint64_t file_number, Cache::Handle** handle);
  ...
  static TableReader* GetTableReaderFromHandleWithCache(Cache::Handle* handle, Cache* cache_);
