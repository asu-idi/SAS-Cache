...
class CompactionJob {
 public:
  ...
  // void PrefetchInBackground(std::vector<Slice> keys, std::vector<Slice> min_keys, std::vector<Slice> max_keys, int out_put_level);

  void PrefetchInBackground(Slice& min_keys, Slice& max_keys, int out_put_level);

  void RecordInvalidFiles(std::vector<uint64_t>& files);

  void EvictInvalidBlocks(std::vector<CacheKey>& blocks);

  void CleanCompactionWrapper();

  void CompactionReplacement();
