...
void CompactionJob::PrefetchInBackground(Slice& smallest_data, Slice& largest_data, int out_put_level) {
    std::string dbname = dbname_;
    // here should be shared_ptr because we need to use it in another thread
    auto secondary_cache = compact_->compaction->immutable_options()->secondary_cache;

    std::vector<char> min_keys(smallest_data.data(), smallest_data.data() + smallest_data.size());
    std::vector<char> max_keys(largest_data.data(), largest_data.data() + largest_data.size());
    std::thread(&SecondaryCache::KeyRangePrefetch, secondary_cache, dbname, min_keys, max_keys, out_put_level, secondary_cache).detach();
}

void CompactionJob::RecordInvalidFiles(std::vector<uint64_t>& files) {
    std::string dbname = dbname_;
    auto secondary_cache = compact_->compaction->immutable_options()->secondary_cache;
    secondary_cache->RecordInvalidFiles(dbname, files);
}

void CompactionJob::EvictInvalidBlocks(std::vector<CacheKey>& blocks) {
    auto secondary_cache = compact_->compaction->immutable_options()->secondary_cache;
    std::thread(&SecondaryCache::EvictInvalidBlocks, secondary_cache, blocks).detach();
}
...
void CompactionJob::CleanCompactionWrapper() {
  CleanupCompaction();
}

void CompactionJob::CompactionReplacement() {
  auto deleted_files = compact_->compaction->inputs();
  if(compact_->compaction->immutable_options()->enable_flash_admission_list) {
    std::vector<uint64_t> invalid_files;
    // std::cout << "compaction begin: " << std::endl;
    for(auto it = deleted_files->begin(); it != deleted_files->end(); ++it) {
      auto files_in_level = (*it).files;
      // std::cout << "(compaction)level: " << (*it).level << std::endl;
      for(auto iit = files_in_level.begin(); iit != files_in_level.end(); ++iit) {
        // std::cout << "(compaction)file_number: " << (*iit)->fd.GetNumber() << std::endl;
        invalid_files.push_back((*iit)->fd.GetNumber());
      }
    }
    RecordInvalidFiles(invalid_files);
  }

  if(compact_->compaction->immutable_options()->enable_flash_evict_blocks) {
    std::vector<CacheKey> keys;
    for(auto it = deleted_files->begin(); it != deleted_files->end(); ++it) {
      auto files_in_level = (*it).files;
      for(auto iit = files_in_level.begin(); iit != files_in_level.end(); ++iit) {
        TableReader* t = (*iit)->fd.table_reader; 
        Cache::Handle* handle = nullptr;
        if(t == nullptr) {
          TableCache::GetTableByFileNum(table_cache_.get(), (*iit)->fd.GetNumber(), &handle);
          t = TableCache::GetTableReaderFromHandleWithCache(handle, table_cache_.get());
        }
        Status status = t->GetBlockKeysInOneFile(keys);
      }
    }
    // auto start_time = std::chrono::high_resolution_clock::now();
    // EvictInvalidBlocks(keys);
    // auto end_time = std::chrono::high_resolution_clock::now();
    // auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
    // std::cout << "Evict_consume_Time: " << duration << "us" << std::endl;
  }
  
  if(compact_->compaction->immutable_options()->enable_flash_prefetch_files) {
    // auto start_time = std::chrono::high_resolution_clock::now();
    std::vector<const CompactionOutputs::Output*> files_output;
    int out_put_level = compact_->compaction->output_level();
    for (const auto& state : compact_->sub_compact_states) {
      for (const auto& output : state.GetOutputs()) {
          files_output.emplace_back(&output);
      }
    ...
    for(auto it = files_output.begin(); it != files_output.end(); ++it) {
      auto file_metadata = (*it)->meta;
      Slice smallest = Slice(file_metadata.smallest.user_key().data(), file_metadata.smallest.user_key().size());
      Slice largest = Slice(file_metadata.largest.user_key().data(), file_metadata.largest.user_key().size());
      (void)smallest;
      (void)largest; 
      // PrefetchInBackground(smallest, largest, out_put_level);
    }
    // auto end_time = std::chrono::high_resolution_clock::now();
    // auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
    // std::cout << "Prefetch_consume_Time: " << duration << "us" << std::endl;
  }
