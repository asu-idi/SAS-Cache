...
Status DBImplSecondary::GetBlockInLevel(const ReadOptions& read_options,
                ColumnFamilyHandle* column_family, const Slice& key, uint64_t level,
                std::vector<Slice>& block_contents) {
  return DBImpl::GetBlockInLevel(read_options, column_family, key, level, block_contents);
}

Status DBImplSecondary::GetBlockInLevelRange(const ReadOptions& read_options,
                ColumnFamilyHandle* column_family, const Slice& key, uint64_t low_level, uint64_t high_level,
                std::vector<Slice>& block_contents) {
  return DBImpl::GetBlockInLevelRange(read_options, column_family, key, low_level, high_level, block_contents);
}

Status DBImplSecondary::GetAllBlocksInLevel(const ReadOptions& read_options,
                ColumnFamilyHandle* column_family, uint64_t level,
                std::vector<Slice>& block_contents) {
  return DBImpl::GetAllBlocksInLevel(read_options, column_family, level, block_contents);
}

Status DBImplSecondary::GetBlockInKeyRange(const ReadOptions& read_options,
                        ColumnFamilyHandle* column_family, uint64_t level, Slice min_key, Slice max_key, 
                        std::vector<Slice>& block_contents) {
  return DBImpl::GetBlockInKeyRange(read_options, column_family, level, min_key, max_key, block_contents);
}
