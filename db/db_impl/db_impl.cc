...
Status DBImpl::GetBlockInLevel(const ReadOptions& read_options,
                        ColumnFamilyHandle* column_family, const Slice& key, uint64_t level,
  ...
  Status s;
  PinnableSlice* pinnable_val = nullptr;
  MergeContext merge_context;
  GetWithTimestampReadCallback read_cb(snapshot);
  std::string* ts = nullptr;
  sv->current->GetBlockInLevel(
      read_options, lkey, pinnable_val, ts, &s, &merge_context,
      &max_covering_tombstone_seq, &pinned_iters_mgr, block_contents ,level, 
      /*value_found*/ nullptr, /*seq*/ nullptr, &read_cb,
      /*is_blob*/ nullptr,
      /*do_merge*/ true);
...
Status DBImpl::GetAllBlocksInLevel(const ReadOptions& read_options,
                        ColumnFamilyHandle* column_family, uint64_t level,
                        std::vector<Slice>& block_contents) {
  
  // The key is unuse , just for pedding
  Slice key;
...
Status DBImpl::GetBlockInLevelRange(const ReadOptions& read_options,
                        ColumnFamilyHandle* column_family, const Slice& key, uint64_t low_level, uint64_t high_level, 
                        std::vector<Slice>& block_contents) {
  Status s;
  for(uint64_t i = low_level; i <= high_level; ++i) {
    s = GetBlockInLevel(read_options,column_family, key, i, block_contents);
  }
  return s;
}


Status DBImpl::GetBlockInKeyRange(const ReadOptions& read_options,
                        ColumnFamilyHandle* column_family, uint64_t level, Slice min_key, Slice max_key, 
                        std::vector<Slice>& block_contents) {
  ...
  LookupKey min_lkey(min_key, snapshot, read_options.timestamp);
  LookupKey max_lkey(max_key, snapshot, read_options.timestamp);
  PinnedIteratorsManager pinned_iters_mgr;
  SequenceNumber max_covering_tombstone_seq = 0;
  Status s;
  PinnableSlice* pinnable_val = nullptr;
  ...
  GetWithTimestampReadCallback read_cb(snapshot);
  std::string* ts = nullptr;
  sv->current->GetBlocksWithKeyRange(
      read_options, min_lkey, max_lkey, pinnable_val, ts, &s, &merge_context,
      &max_covering_tombstone_seq, &pinned_iters_mgr, block_contents ,level, 
      /*value_found*/ nullptr, /*seq*/ nullptr, &read_cb,
      /*is_blob*/ nullptr,
      /*do_merge*/ true);
