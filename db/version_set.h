...
class Version {
 public:
  ...
  void GetBlockInLevel(const ReadOptions&, const LookupKey& key, PinnableSlice* value,
           ...
           bool* value_found = nullptr, SequenceNumber* seq = nullptr, ReadCallback* callback = nullptr,
           bool* is_blob = nullptr, bool do_merge = true);

  void GetBlocksWithKeyRange(const ReadOptions&, const LookupKey& min_key ,const LookupKey& max_key, PinnableSlice* value,
           std::string* timestamp, Status* status, MergeContext* merge_context,
           SequenceNumber* max_covering_tombstone_seq,
           PinnedIteratorsManager* pinned_iters_mgr, std::vector<Slice>& str_block_contents, uint64_t level,
           bool* value_found = nullptr, SequenceNumber* seq = nullptr, ReadCallback* callback = nullptr,
...
/**
 * @brief The key here is unuse , just for pedding.
 */
  void GetAllFilesInLevel(const ReadOptions&, const LookupKey& key, PinnableSlice* value,
           std::string* timestamp, Status* status, MergeContext* merge_context,
           SequenceNumber* max_covering_tombstone_seq,
           PinnedIteratorsManager* pinned_iters_mgr, std::vector<Slice>& str_block_contents, uint64_t level,
           bool* value_found = nullptr, SequenceNumber* seq = nullptr, ReadCallback* callback = nullptr,
