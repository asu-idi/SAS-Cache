...
class FilePicker {
 public:
  ...
  /**
   * @brief Get the file containing the key in a level
   * 
   * @param level 
   * @return FdWithKeyRange* 
   */
  FdWithKeyRange* GetFileInLevel(uint64_t level) {
    if(static_cast<unsigned int>(level) >= num_levels_) {
      return nullptr;
    }
    auto curr_file_level_local_ = &(*level_files_brief_)[level];
    int cmp_largest = -1;
    if(level == 0) {
      // level0 : need to deal with the case which there are multiple files because level0 is unordered
      while(level_zero_index < curr_file_level_local_->num_files) {
        auto f = &curr_file_level_local_->files[level_zero_index];
        int cmp_smallest = user_comparator_->CompareWithoutTimestamp(
            user_key_, ExtractUserKey(f->smallest_key));
        if (cmp_smallest >= 0) {
          cmp_largest = user_comparator_->CompareWithoutTimestamp(
        ...
        // whether success or false we all need to add this.
        level_zero_index++;
        // Key falls in the current file's range
        if (cmp_smallest >= 0 && cmp_largest <= 0) {
          return f;
        }
      ...
      while(index < curr_file_level_local_->num_files) {
        // TODO: implement binary search
        auto f = &curr_file_level_local_->files[index];
        int cmp_smallest = user_comparator_->CompareWithoutTimestamp(
            user_key_, ExtractUserKey(f->smallest_key));
        if (cmp_smallest >= 0) {
          cmp_largest = user_comparator_->CompareWithoutTimestamp(
        ...
        if (cmp_smallest >= 0 && cmp_largest <= 0) {
          ...
          // smaller the smallest key, so break
          break;
        }
  ...
  void PrintAllFiles() {
    for(unsigned int i = 0; i < num_levels_; i++) {
      auto curr_file_level_local_ = &(*level_files_brief_)[i];
      std::cout << "Level: " << i << std::endl;
      for(unsigned int j = 0; j < curr_file_level_local_->num_files; j++) {
        auto f = &curr_file_level_local_->files[j];
        std::cout << "file number: " << f->fd.GetNumber() << ", smallest key: " << f->smallest_key.ToString() << ", largest key: " << f->largest_key.ToString() << std::endl;
      }
    }
  }

   void GetFileInLevelWithKeyRange(uint64_t level, Slice min_key, Slice max_key, std::vector<FdWithKeyRange*>& fds) {
    // PrintAllFiles();
    // std::cout << "Get File: level, min_key, max_key: " << level << ", " << min_key.ToString() << ", " << max_key.ToString() << std::endl;
    if(static_cast<unsigned int>(level) >= num_levels_) {
      return;
    }
    auto curr_file_level_local_ = &(*level_files_brief_)[level];
    ...
    if(level == 0) {
      // level0 : need to deal with the case which there are multiple files because level0 is unordered
      unsigned int i = 0;
      while(i < curr_file_level_local_->num_files) {
        auto f = &curr_file_level_local_->files[i];
        int cmp_smallest = user_comparator_->CompareWithoutTimestamp(
            min_key, ExtractUserKey(f->smallest_key));
        if (cmp_smallest >= 0) {
          cmp_largest = user_comparator_->CompareWithoutTimestamp(
              max_key, ExtractUserKey(f->largest_key));
        }

        i++;
        // Key falls in the current file's range, Then push this file into fds
        if (cmp_smallest >= 0 && cmp_largest <= 0) {
          fds.push_back(f);
        }
      }
      ...
      unsigned int max_index = curr_file_level_local_->num_files - 1;
      int max_cmp_largest = -1;
      while(index < curr_file_level_local_->num_files) {
        // std::cout << "File[index], min_index, max_index: " << index << ", " << min_index << ", " << max_index << std::endl;
        // std::cout << "file number: " << curr_file_level_local_->files[index].fd.GetNumber() << std::endl;
        // std::cout << "File[index], min_key, max_key: " << curr_file_level_local_->files[index].smallest_key.ToString() << ", " << curr_file_level_local_->files[index].largest_key.ToString() << std::endl;
        auto f = &curr_file_level_local_->files[index];
        int cmp_smallest = user_comparator_->CompareWithoutTimestamp(
            min_key, ExtractUserKey(f->smallest_key));
        if (cmp_smallest >= 0) {
          cmp_largest = user_comparator_->CompareWithoutTimestamp(
              min_key, ExtractUserKey(f->largest_key));
        }

        int max_cmp_smallest = user_comparator_->CompareWithoutTimestamp(
            max_key, ExtractUserKey(f->smallest_key));
        if (max_cmp_smallest >= 0) {
          max_cmp_largest = user_comparator_->CompareWithoutTimestamp(
              max_key, ExtractUserKey(f->largest_key));
        }

        // std::cout << "cmp_smallest, cmp_largest, max_cmp_smallest, max_cmp_largest: " << cmp_smallest << ", " << cmp_largest << ", " << max_cmp_smallest << ", " << max_cmp_largest << std::endl;

        // Key falls in the current file's range
        if (cmp_smallest >= 0 && cmp_largest <= 0) {
          min_index = index;
        }
        if (max_cmp_smallest >= 0 && max_cmp_largest <= 0) {
          max_index = index;
        }
      ...
      for(auto i = min_index; i <= max_index; ++i) {
        std::cout << "get_file: " << i << ", " << curr_file_level_local_->files[i].fd.GetNumber() << ", " << curr_file_level_local_->files[i].fd.GetPathId() << std::endl;
        fds.push_back(&curr_file_level_local_->files[i]);
      }
    }
    // Search ended.
    return;
  ...
  /**
   * @brief Get the all files in one certain level
   * @param level 
   * @return std::vector<FdWithKeyRange*>
   */

  bool GetAllFilesInlevel(uint64_t level, std::vector<FdWithKeyRange*>& files) {
    if(static_cast<unsigned int>(level) >= num_levels_) {
      return false;
    }
    auto curr_file_level_local_ = &(*level_files_brief_)[level];
    for(unsigned int i = 0;i < curr_file_level_local_->num_files; ++i) {
      auto f = &curr_file_level_local_->files[i];
 ...
 private:
  ...
  unsigned int level_zero_index; // record the index in level0 for function GetFileInLevel
...
void Version::GetBlockInLevel(const ReadOptions& read_options, const LookupKey& k,
                  PinnableSlice* value, std::string* timestamp, Status* status,
                  MergeContext* merge_context,
                  SequenceNumber* max_covering_tombstone_seq,
                  PinnedIteratorsManager* pinned_iters_mgr, std::vector<Slice>& str_block_contents, uint64_t level, 
                  bool* value_found, SequenceNumber* seq, ReadCallback* callback,
                  bool* is_blob, bool do_merge) {
  Slice ikey = k.internal_key();
  Slice user_key = k.user_key();
  ...
  GetContext get_context(
      user_comparator(), merge_operator_, info_log_, db_statistics_,
      status->ok() ? GetContext::kNotFound : GetContext::kMerge, user_key,
      do_merge ? value : nullptr, nullptr, do_merge ? timestamp : nullptr, value_found,
  ...
  FilePicker fp(user_key, ikey, &storage_info_.level_files_brief_,
                ...
                &storage_info_.file_indexer_, user_comparator(),
                internal_comparator());
  //TODO: in level0 : use a vector
  FdWithKeyRange* f = fp.GetFileInLevel(level);

  // The format of this string :
  // block_size(size_t)+ '*' + index_size(size_t) + '*' 
  //      + filter_size(size_t) + '*' + DelRange_size(size_t)
  // This is the last elem in the str_block_content vector
  // is used to divid the different types of blocks for the upper layer.
  std::string RecordBlockSize = "";
  size_t BlockSize = 0;

  if (f != nullptr) {

    get_context.set_whether_get_block(true);
    *status = table_cache_->GetBlockInLevel(
        read_options, *internal_comparator(), *f->file_metadata, ikey,
        &get_context, mutable_cf_options_.prefix_extractor,
        cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
        IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
                        fp.IsHitFileLastInLevel()),
        fp.GetHitFileLevel(), max_file_size_for_l0_meta_pin_);
    RecordBlockSize += std::to_string(get_context.get_str_block_size());
    BlockSize = get_context.get_str_block_size();
    RecordBlockSize += "*";

    if(read_options.get_index_block) {
      *status = table_cache_->GetIndexBlock(
          read_options, *internal_comparator(), *f->file_metadata, ikey,
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
    ...
    RecordBlockSize += std::to_string(get_context.get_str_block_size() - BlockSize);
    BlockSize = get_context.get_str_block_size();
    RecordBlockSize += "*";

    if(read_options.get_filter_block) {
      *status = table_cache_->GetFilterBlock(
          read_options, *internal_comparator(), *f->file_metadata, ikey,
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
    ...
    if(read_options.get_del_range_block ){
      get_context.set_whether_get_del_range_block(true);
      *status = table_cache_->GetDelRangeBlock(
          read_options, *internal_comparator(), *f->file_metadata, ikey,
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
    ...
    get_context.set_whether_get_del_range_block(false);
    RecordBlockSize += std::to_string(get_context.get_str_block_size() - BlockSize);

    // TODO: examine the behavior for corrupted key
    auto str_contents = get_context.get_str_block_content();
    for(auto it = str_contents.begin(); it != str_contents.end(); ++it) {
      auto content = *it;
      str_block_contents.push_back(std::move(content));
    }
    // Non-empty : then push this. 
    // If it is empty : no need to push this.
    if(!str_block_contents.empty()) {
      str_block_contents.push_back(RecordBlockSize);
    }
  }
}

void Version::GetAllFilesInLevel(const ReadOptions& read_options, const LookupKey& k,
                  PinnableSlice* value, std::string* timestamp, Status* status,
                  MergeContext* merge_context,
                  SequenceNumber* max_covering_tombstone_seq,
                  PinnedIteratorsManager* pinned_iters_mgr, std::vector<Slice>& str_block_contents, uint64_t level, 
  ...
  FilePicker fp(user_key, ikey, &storage_info_.level_files_brief_,
                ...
                internal_comparator());
  FdWithKeyRange* f;
  std::vector<FdWithKeyRange*> all_files;
  if(!fp.GetAllFilesInlevel(level, all_files)) {
  }
  ...
  for(auto it = all_files.begin(); it != all_files.end(); ++it){
    f = *it;
    *status = table_cache_->GetAllBlocksInOneFile(
        read_options, *internal_comparator(), *f->file_metadata, ikey,
        &get_context, mutable_cf_options_.prefix_extractor,
        cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
        IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
...
void Version::GetBlocksWithKeyRange(const ReadOptions& read_options, const LookupKey& min_key ,const LookupKey& max_key,
                  PinnableSlice* value, std::string* timestamp, Status* status,
                  MergeContext* merge_context,
                  SequenceNumber* max_covering_tombstone_seq,
                  PinnedIteratorsManager* pinned_iters_mgr, std::vector<Slice>& str_block_contents, uint64_t level, 
                  bool* value_found, SequenceNumber* seq, ReadCallback* callback,
                  bool* is_blob, bool do_merge) {
  Slice min_ikey = min_key.internal_key();
  Slice min_user_key = min_key.user_key();

  Slice max_ikey = max_key.internal_key();
  Slice max_user_key = max_key.user_key();

  FilePicker fp(min_user_key, min_ikey, &storage_info_.level_files_brief_,
                storage_info_.num_non_empty_levels_,
                &storage_info_.file_indexer_, user_comparator(),
                internal_comparator());
  FdWithKeyRange* f;
  std::vector<FdWithKeyRange*> all_files;
  fp.GetFileInLevelWithKeyRange(level, min_user_key, max_user_key, all_files);
  ...
  GetContext get_context(
      user_comparator(), merge_operator_, info_log_, db_statistics_,
      status->ok() ? GetContext::kNotFound : GetContext::kMerge, min_user_key,
      do_merge ? value : nullptr, nullptr, do_merge ? timestamp : nullptr, value_found,
      merge_context, do_merge, max_covering_tombstone_seq, clock_, seq,
      merge_operator_ ? pinned_iters_mgr : nullptr, callback, is_blob_to_use,
      tracing_get_id, &blob_fetcher);
  ...
  if(level == 0) {
    for(auto it = all_files.begin(); it != all_files.end(); ++it){
      f = *it;
      *status = table_cache_->GetAllBlocksWithKeyRange(
          read_options, *internal_comparator(), *f->file_metadata, min_ikey, max_ikey, true, true, 
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
                          fp.IsHitFileLastInLevel()),
    ...
    if(size == 0) {
      std::cout << "enter file size 0" << std::endl;
      return;
    }
    else if(size == 1) {
      std::cout << "enter file size 1" << std::endl;
      for(auto it = all_files.begin(); it != all_files.end(); ++it){
        f = *it;
        *status = table_cache_->GetAllBlocksWithKeyRange(
            read_options, *internal_comparator(), *f->file_metadata, min_ikey, max_ikey, true, true,
      ...
      std::cout << "enter file size 2" << std::endl;
      //file contains the min_key
      f = all_files[0];
      *status = table_cache_->GetAllBlocksWithKeyRange(
          read_options, *internal_comparator(), *f->file_metadata, min_ikey, max_ikey, true, false,
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
                          fp.IsHitFileLastInLevel()),
      ...
      //file contains the max_key
      f = all_files[1];
      *status = table_cache_->GetAllBlocksWithKeyRange(
          read_options, *internal_comparator(), *f->file_metadata, min_ikey, max_ikey, false, true,
          &get_context, mutable_cf_options_.prefix_extractor,
          cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
          IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
                          fp.IsHitFileLastInLevel()),
      ...
      *status = table_cache_->GetAllBlocksWithKeyRange(
      ...
      for(int i = 1; i < size - 1; ++i) {
        f = all_files[i];
        *status = table_cache_->GetAllBlocksInOneFile(
            read_options, *internal_comparator(), *f->file_metadata, min_ikey,
            &get_context, mutable_cf_options_.prefix_extractor,
            cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()),
            IsFilterSkipped(static_cast<int>(fp.GetHitFileLevel()),
                            fp.IsHitFileLastInLevel()),
  ...
  // std::cout << "block size: " << get_context.get_str_block_size() << std::endl;
  auto str_contents = get_context.get_str_block_content();
  for(auto it = str_contents.begin(); it != str_contents.end(); ++it) {
    auto content = *it;
    if(content != "") {
      Slice block = Slice(content.data(), content.size());
      str_block_contents.emplace_back(std::move(block));
      if(content != str_block_contents.back()) {
        std::cout << "Error: version_set get failed" << std::endl;
      }
    }
  }
}
